...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r ".vout[$UTXO_VOUT_INDEX].value" | awk '{printf "%.0f", $1 * 100000000}')
...
FEE_SATS=$(echo "$TX_SIZE * $FEE_RATE" | bc)
...
CHANGE_AMOUNT=$(echo "$UTXO_VALUE - $PAYMENT_AMOUNT - $FEE_SATS" | bc)
...
CHILD_SEND_AMOUNT=$(echo "$CHANGE_AMOUNT - $CHILD_FEE_SATS" | bc)
